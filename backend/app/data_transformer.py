...
_ROUTES_CACHE = {}
_ROUTES_CACHE_TTL = 300  # 5-minute in-memory cache for ultra-fast 2ms route queries
...
def get_routes(vessel_id=None, dest_id=None, dest_lat=None, dest_lon=None, dest_name=None):
    ...
    cache_key = (
        ...
        round(float(dest_lat), 3) if dest_lat is not None else None,
        round(float(dest_lon), 3) if dest_lon is not None else None,
        str(dest_name) if dest_name else None
    )
    now = time.time()
    ...
    if vessel_id:
        ...
        routes = []
        for v in vessels:
            routes.extend(routing_engine.generate_routes(v, dest_override=dest_override, dest_name=dest_name))
